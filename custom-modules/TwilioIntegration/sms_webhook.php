<?php
/**
 * SMS Webhook Handler - Standalone
 *
 * Twilio Messaging Webhook URL:
 * https://customer-relations.boomershub.com/legacy/modules/TwilioIntegration/sms_webhook.php
 *
 * Method: HTTP POST
 */
...
$legacyRoot = null;
$possiblePaths = [
    dirname(__FILE__) . '/../..',           // modules/TwilioIntegration -> legacy
    dirname(__FILE__, 3),                    // Same as above for PHP 7+
    '/bitnami/suitecrm/public/legacy',       // Direct path in container
    '/var/www/html/legacy',                  // Alternative path
];

foreach ($possiblePaths as $path) {
    $configFile = rtrim($path, '/') . '/config.php';
    if (file_exists($configFile)) {
        $legacyRoot = rtrim($path, '/');
        file_put_contents($logFile, "[$timestamp] Found config.php at: $legacyRoot\n", FILE_APPEND);
        break;
    }
}

if (!$legacyRoot) {
    file_put_contents($logFile, "[$timestamp] ERROR: Could not find config.php in any path\n", FILE_APPEND);
    file_put_contents($logFile, "  Tried paths: " . json_encode($possiblePaths) . "\n\n", FILE_APPEND);

    // Return success anyway so Twilio doesn't retry
    header('Content-Type: application/xml');
    echo '<?xml version="1.0" encoding="UTF-8"?>';
    echo '<Response></Response>';
    exit;
...
// Change to SuiteCRM directory and load it
chdir($legacyRoot);
